...
- **OpenTDF / KAS encryption** — Tool outputs and SwarmKit role data can be wrapped in TDF envelopes; policy decisions are enforced by a Key Access Server before decryption. Built on `opentdf-rs` behind the optional `kas` build feature. Rewrap (decryption) needs a reachable KAS — the defaults are Arkavo-hosted (`identity.arkavo.net`, `platform.arkavo.net`), but the endpoint is configurable, so a KAS self-hosted in-environment keeps encryption and policy enforcement fully functional offline / air-gapped.
