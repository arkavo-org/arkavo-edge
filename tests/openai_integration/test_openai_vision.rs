...
#[tokio::test]
#[ignore = "Requires OPENAI_API_KEY environment variable and GPT-4o access"]
async fn test_gpt4o_vision_basic() {
    ...
    let response = provider.complete(messages).await
        .expect("Failed to get response from GPT-4o with image");

    println!("GPT-4o vision response: {}", response);
...
async fn test_non_vision_model_with_image_fails() {
    ...
    let config = OpenAIConfig {
        api_key,
        base_url: "https://api.openai.com/v1".to_string(),
        model: "gpt-3.5-turbo".to_string(), // This model doesn't support vision
        organization_id: None,
        api_version: None,
        is_azure: false,
    };
    ...
    // GPT-3.5-turbo doesn't support images, so this should either fail or ignore the image
    match result {
        Ok(response) => {
