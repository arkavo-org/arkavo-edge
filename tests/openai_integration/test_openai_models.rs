...
async fn test_gpt_4o_model() {
    ...
    let provider = OpenAIProvider::new(config)
        .expect("Failed to create OpenAI provider for GPT-4o");
    ...
    let response = provider.complete(messages).await
        .expect("Failed to get response from GPT-4o");
    let duration = start.elapsed();

    println!("GPT-4o response time: {:?}", duration);
    println!("GPT-4o response: {}", response);
...
async fn test_gpt_4_turbo_model() {
    ...
    let provider = OpenAIProvider::new(config)
        .expect("Failed to create OpenAI provider for GPT-4-turbo");
    ...
    let start = Instant::now();
    let response = provider.complete(messages).await
        .expect("Failed to get response from GPT-4-turbo");
    let duration = start.elapsed();

    println!("GPT-4-turbo response time: {:?}", duration);
    println!("GPT-4-turbo haiku:\n{}", response);
...
async fn test_gpt_35_turbo_model() {
    ...
    let provider = OpenAIProvider::new(config)
        .expect("Failed to create OpenAI provider for GPT-3.5-turbo");
    ...
    let start = Instant::now();
    let response = provider.complete(messages).await
        .expect("Failed to get response from GPT-3.5-turbo");
    let duration = start.elapsed();

    println!("GPT-3.5-turbo response time: {:?}", duration);
    println!("GPT-3.5-turbo response: {}", response);
...
async fn test_context_window_handling() {
    ...
    let config = OpenAIConfig {
        api_key,
        base_url: "https://api.openai.com/v1".to_string(),
        model: "gpt-4-turbo".to_string(), // 128k context window
        organization_id: None,
        api_version: None,
        is_azure: false,
    };
