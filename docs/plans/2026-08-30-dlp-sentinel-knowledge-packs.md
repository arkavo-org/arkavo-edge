...
- Ingest and chunk with source metadata retained. The taxonomy map is prescribed and generalized (six DLP labels; wrap emits `https://attr.arkavo.com/attr/<def>/value/<val>`). Sample corpus is Hugging Face `opioidarchive/oida-qa`. Project binds to IDL collection via an offline parquet join on `PDF_NAME`; a miss is `project/value/unknown` and is excluded from partitioned adapters. Derived artifacts take max(clearance) and union(conjunctive attrs). Data-derived tags take three wrap-time exits chosen by the map's `derived` table: signed assertion (tagger key, not DEK-bound) always; `https://derived.arkavo.com/attr/<def>/value/<val>` only for declared ALL_OF values above threshold; omit from dataAttributes below threshold. Promotion tightens subject mapping on already-stamped values and requires a rewrap. Public negatives are topic-matched FDA/SEC/trial/press text from the same companies.
