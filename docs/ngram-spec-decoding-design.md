...
| `arkavo-llm` | Generation loop in `llamacpp_streaming/` (split into `mod.rs` dispatcher + `spec.rs` for both pooled and non-pooled spec functions) wraps decode with `begin → loop { draft → batch_eval → walk accept-prefix → emit → accept(n) }`. Reads `use_spec_decoding` from `SamplingConfig`; emits raw `n_draft` / `n_accepted` / `spec_bypassed` in `InferenceTiming`. Bypasses spec when grammar or additional stop sequences are active (grammar+spec parity is a deferred follow-up). No thresholds, no env reads, no policy. |
| `SamplingConfig` (`arkavo-llm`) | Gains `use_spec_decoding: bool`. Router writes the per-request flag via `sampling_config_for(model, store, use_spec_decoding)` when instantiating the provider. A `CompletionOptions` struct also exists with the same field for future trait widening, but is not currently on the `Provider` trait surface — the per-call flag flows through `SamplingConfig` instead. |
...
| Grammar interaction (tool calls must still be grammar-constrained) | Streaming layer **bypasses** spec when grammar or additional_stops is active. The bypass reason is captured in `InferenceTiming.spec_bypassed` (`"grammar_active"` / `"stops_active"`), and the router skips updating per-model stats for bypassed calls so the workload-vs-model signal stays clean. Grammar+spec parity (advancing grammar state per accepted draft) is a deferred follow-up; until it lands, tool-call paths get baseline behavior — correctness preserved, no Infer2 speedup from spec on those requests. |
