...
- **Folds in:** draft PR #626 (`github-ops-kit` manifest)
...
Let a swarm of agents **monitor GitHub PRs and review them**, end-to-end, using the existing SwarmKit specialization model: roles + their MCP-tool grants are bundled and assigned to empty awaiting agents on the mesh, which then do the work. The worked example is the `github-ops-kit` (all four roles live): a `dispatcher` discovers PRs and routes them; a `pr_reviewer` posts a code review; a `pr_test_runner` runs whatever tests/checks the swarm decides a given PR needs; a `repo_maintainer` performs repo-scoped housekeeping.

**Roles declare capability, not procedure (emergent behavior).** The manifest gives each role a purpose and a set of granted tools — it does *not* prescribe a fixed routine. Whether a PR needs a full regression, targeted tests, or none is decided by the agents at runtime through coordination (dispatcher triage, maintainer/reviewer signals, handoffs), not hard-coded. This matches how the rest of the system works: let the agents decide, don't encode the rules.
...
- In-process MCP tool registry — `arkavo-mcp-tools` (`registry.rs`): `git_*`, `github_*`, `gh_pr_review`:220, `gh_checks`, `github_ci_status`, `test_run`:241 (`TestRunnerTool` — runs the project's test suite), `code_review`:304; `test_*` auto-categorized "Testing":655. In-process `Tool` impls; `McpToolGrant.server` is a logical label.
- GitHub ops — `arkavo-github` (`poller.rs`/`org_polling.rs` poll **issues** on a 5-min cron; `operations.rs` `create_pr`/`list_prs`/`merge_pr`; `issue_ops.rs` `post_comment`; `app_auth.rs` GitHub App / PAT).
...
4. **Post-specialize role execution** — the agent loop is continuous-advisory; nothing makes a specialized agent *do its role* (poll, review, test, maintain).
...
| D2 | **One branch off `main`**, folding #626 in. | No stacked PRs; complete integrated feature. |
| D3 | **PR poller is an MCP tool** (`github_pr_watch`) declared in the manifest, granted to `dispatcher`; bundled to an empty awaiting agent. No bespoke runner. | Reuses the specialization model; keeps GitHub specifics out of core. |
...
| D6 | **Roles declare capability, not procedure — no hard-coded gates.** A role's manifest entry is purpose + tool grants; the swarm decides per-PR what to do. `pr_test_runner` is granted the existing general test/check tools (`test_run`, `gh_checks`, `github_ci_status`); whether/what it tests is agent-determined. **No model-regression eval gate** (`run_eval`/#625 is a narrow, specialized concern, out of scope for normal PR review). | The point of the swarm is emergent behavior; hard-coding a fixed gate defeats it. Uses only existing tools — no new crates, no llama.cpp, no fold-in of #625. |
...
agent: fetch bundle blob via ticket (IrohTransport.fetch_bytes) → unwrap_bundle (existing)
       → persona + grants + scoped tokens → GRANT-FILTERED registry → runs its role
        ├─ dispatcher    : github_pr_watch (poll ORG) → triage → route each new/changed PR
        ├─ pr_reviewer   : git_diff, gh_pr_review        → posts review        (own grant)
        ├─ pr_test_runner: test_run, gh_checks           → runs tests the swarm calls for, posts result
        └─ repo_maintainer: git_*, github_issue_*, github_pr_create → repo-scoped writes (own grant)
```

The review/result is posted **by the role agents through their own granted tools** — no central result-collection plumbing for the primary outcome. The transport returns only lifecycle/status. What `pr_test_runner` actually runs (or skips) is decided by swarm coordination, not the manifest.
...
| `pr_test_runner` | operator / coordination | Gemma-4 E2B | `test_run` (none), `gh_checks` (delegated), `github_ci_status` (delegated) |
...
Least-privilege boundaries (asserted by test): reviewer cannot `github_pr_create`; maintainer cannot `gh_pr_review`; `pr_test_runner` can run tests/checks but cannot post reviews.

### Component map

| Crate | Change |
|---|---|
| `arkavo-github` | New PR ops: `get_pr`, `list_open_prs(state=open, sort=updated)`, PR diff/files; PR cursor/dedup state (head-SHA) parallel to issue state. |
...
| `arkavo-cli` | `arkavo swarm apply <manifest> --org <ORG> [--once] [--interval <m>]` (prod `apply_kit` caller). |
| `examples/github-ops-kit` | **Fold #626** manifest + `github_ops_kit_validates` test; add the `github_pr_watch` grant to `dispatcher`; `pr_test_runner` grants `test_run` (was the eval gate). |

## Workstreams (build order, all on this branch)

- **WS-A — Fold-in.** Vendor the `github-ops-kit` manifest + validation test (#626); adjust grants (`dispatcher` gets `github_pr_watch`; `pr_test_runner` grants `test_run`). Independent; lands first. (No `arkavo-eval`/#625 fold-in — dropped per D6.)
- **WS-B — GitHub PR capability.** `arkavo-github` PR ops + state cursor; `github_pr_watch` MCP tool. Independent of the mesh.
...
- **Error semantics:** infra failures (GitHub API down, checkout/build error) must be distinguished from substantive findings — never post a misleading review or check verdict on infra failure; surface the infra error instead. PR head-SHA cursor prevents re-review of unchanged PRs; a new push re-triggers. Budget: $0 cost cap (local only), 1800s/120k per the manifest, enforced per role via `derive_arp_for_role`.
...
- **WS-A:** manifest validation (`github_ops_kit_validates`, folded #626) — parses, cross-block `validate()`, kit-id round-trip, least-privilege boundaries; `dispatcher` grants `github_pr_watch`; `pr_test_runner` grants `test_run`.
- **WS-B:** `github_pr_watch` — new PR detected, head-SHA change re-triggers, unchanged skipped (mocked GitHub API).
...
- **WS-E:** end-to-end — agents announced on the mesh → `swarm apply` → dispatcher polls a mock GitHub → reviewer posts via mocked `gh_pr_review`, runner posts a check via `gh_checks`, maintainer writes scoped to the triggering repo (asserted).
...
- **Model-regression eval gate** (`run_eval` / PR #625) — a narrow, specialized capability, not part of normal PR review. Remains its own separate draft for whenever model-regression gating is actually wanted.
...
- **R4 — `pr_test_runner` execution environment:** running the project's tests needs the PR checked out + a build toolchain on the agent host. Confirm during planning how the runner obtains the working tree (the swarm decides *whether* to test; the host must be *able* to).
...
- Tools: `arkavo-mcp-tools/src/registry.rs` (`test_run`:241, `gh_pr_review`:220, `gh_checks`, `github_ci_status`, Testing:655), `test_runner.rs`, `github_review.rs`, `github_checks.rs`.
- GitHub: `arkavo-github/src/{poller.rs, org_polling.rs, operations.rs, issue_ops.rs, app_auth.rs}`.
- Fold-in: PR #626 (`github-ops-kit`).
