...
/// Reject a backup filename that could escape `.arkavo/backups` once joined
/// onto that directory: empty, containing a path separator, or containing
/// `..`. Backup filenames are always generated by `handle_config_update`
/// itself (`{kit_filename}.{timestamp}.backup`), so a legitimate filename
/// never needs any of these characters — an RPC-supplied `backup_filename`
/// that does is rejected before `handle_config_restore` ever joins it onto
/// the backup directory.
pub(super) fn is_safe_backup_filename(name: &str) -> bool {
...
"#;
    ...
    #[test]
    fn is_safe_backup_filename_accepts_generated_shape() {
        assert!(is_safe_backup_filename(
            "agent.swarmkit.yaml.2026-04-29-120000.backup"
        ));
    }

    #[test]
    fn is_safe_backup_filename_rejects_traversal_and_empty() {
        for bad in [
            "../../etc/hostname",
            "..\\..\\etc\\hostname",
            "../escaped.backup",
            ...
            assert!(!is_safe_backup_filename(bad), "{bad:?} must be rejected");
        }
    }
