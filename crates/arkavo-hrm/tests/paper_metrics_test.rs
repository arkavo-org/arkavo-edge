...
async fn generate_paper_metrics() {
    ...
    println!(
        "- Projected 100-step Savings: ${:.2} vs ${:.2} (99% savings)",
    ...
    println!(
        "- Cost to Restore (Active): ${:.6} / event (High Output Cost)",
        cost_restore_action
    );
    println!(
        "- Break-Even Point: 1 Restore = {:.1} Passive Steps",
        break_even_steps
    );

    let critical_restore_rate = 1.0 / break_even_steps;
    println!(
        "- Critical Restore Rate: {:.1}%",
        critical_restore_rate * 100.0
    );
    println!(
        "  (If agent restores more than once every {:.0} steps, Active is MORE expensive)",
        break_even_steps
    );
