...
use arkavo_mcp_macos::mcp::{
    device_manager::DeviceManager, device_xctest_status::DeviceXCTestStatusManager,
...
async fn test_fast_xctest_verification_cycle() {
    ...
    match DeviceXCTestStatusManager::get_all_devices_with_status(device_manager.clone()).await {
        Ok(devices_with_status) => {
            for device_status in devices_with_status {
                println!(
                    "\n   Device: {} ({})",
                    device_status.device.name, device_status.device.id
                );
                ...
                if let Some(xctest_status) = &device_status.xctest_status {
                    ...
                    println!(
                        "   Bridge Connectable: {}",
                        xctest_status.bridge_connectable
                    );
    ...
    match DeviceXCTestStatusManager::find_best_xctest_device(device_manager.clone()).await {
        Ok(Some(best_device)) => {
            println!(
                "   Best device: {} ({})",
                best_device.device.name, best_device.device.id
            );
...
async fn test_xctest_setup_and_verify() {
    ...
    let booted_device = devices
        .iter()
        .find(|d| d.state == arkavo_mcp_macos::mcp::device_manager::DeviceState::Booted);
    ...
    if let Some(device) = booted_device {
        ...
        let initial_status = XCTestVerifier::verify_device(&device.id)
            .await
                ...
                Ok(result) => {
                        ...
                        .unwrap_or(false)
                    {
                        ...
                        if let Some(device_status) = result.get("device_status") {
                            println!(
                                "   Device status: {}",
                                serde_json::to_string_pretty(device_status).unwrap()
                            );
                        }
                    } else {
                        println!(
        ...
        let final_status = XCTestVerifier::verify_device(&device.id)
            .await
            .expect("Failed to verify device");
