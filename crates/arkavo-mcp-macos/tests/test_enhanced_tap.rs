...
mod tests {
    ...
    use arkavo_mcp_macos::mcp::idb_tap_enhanced::IdbTapEnhanced;
    use arkavo_mcp_macos::mcp::simulator_state_verifier::SimulatorStateVerifier;
    ...
    async fn test_enhanced_tap_with_metrics() {
        ...
        let result = IdbTapEnhanced::tap_with_verification(device_id, 200.0, 400.0, 3).await;

        match result {
            Ok(tap_result) => {
                println!(
                    "Tap succeeded: {}",
                    serde_json::to_string_pretty(&tap_result).unwrap()
                );
                ...
                if let Some(metrics) = IdbCompanionHealth::get_metrics(device_id) {
                    ...
                    println!(
                        "  Success rate: {:.1}%",
                        (metrics.total_taps_succeeded as f64 / metrics.total_taps_attempted as f64)
                            * 100.0
                    );
    ...
    async fn test_tap_sequence_with_verification() {
        ...
        let tap_points = vec![(100.0, 100.0), (200.0, 200.0), (300.0, 300.0)];

        let results = IdbTapEnhanced::tap_sequence_with_verification(
            device_id, tap_points, 500, // 500ms between taps
        )
        .await;
