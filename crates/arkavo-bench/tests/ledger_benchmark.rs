...
async fn benchmark_monorepo_context_limit() {
    ...
    let ptr = conductor.prepare_context_for_burst(&error_log, "Build Errors", &ContextStrategy::Ledger)
        .await
    ...
    println!("Benchmark Passed: Context reduced by {:.4}%", reduction * 100.0);
}
