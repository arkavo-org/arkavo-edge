...
impl TuiScreenshotKit {
    ...
    async fn capture_terminal_text_linux(&self, params: &ScreenshotParams) -> Result<String> {
        ...
        let window_id = if let Some(title) = &params.window_title {
            ...
            if !output.status.success() {
                return Err(ToolError::Mcp("Failed to find terminal window".to_string()));
            }
            ...
            if !output.status.success() {
                return Err(ToolError::Mcp("Failed to get active window".to_string()));
            }
    ...
    fn capture_terminal_image_macos(&self, params: &ScreenshotParams) -> Result<String> {
        ...
        if !output.status.success() {
            return Err(ToolError::Mcp("Failed to capture screenshot".to_string()));
        }

        // Read and encode the image
        let image_data = fs::read(&temp_file)
            .map_err(|e| ToolError::Mcp(format!("Failed to read screenshot: {e}")))?;
