...
impl Tool for QueryStateKit {
    async fn execute(&self, params: Value) -> Result<Value> {
        let entity = params
            .get("entity")
            .and_then(|v| v.as_str())
            .ok_or_else(|| ToolError::Mcp("Missing entity parameter".to_string()))?;
            ...
            _ => {
                ...
                self.state_store.update(
                    ...
                    |current, action_name, action_data| {
                        let mut result = current.cloned().unwrap_or_else(|| serde_json::json!({}));
        ...
        match action {
            ...
            "restore" => {
                let name = params
                    .get("name")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| ToolError::Mcp("Missing snapshot name".to_string()))?;
            ...
            _ => Err(ToolError::Mcp(format!("Invalid action: {action}"))),
        }
    }
