...
impl Tool for FileSystemKit {
    async fn execute(&self, params: Value) -> Result<Value> {
        let action = params["action"]
            .as_str()
            .ok_or_else(|| ToolError::Mcp("Missing 'action' parameter".to_string()))?;

        match action {
            "read_file" => {
                let file_path = params["file_path"]
                    .as_str()
                    .ok_or_else(|| ToolError::Mcp("Missing 'file_path' parameter".to_string()))?;
                ...
                let metadata = fs::metadata(&abs_path)
                    .map_err(|e| ToolError::Mcp(format!("Failed to get file metadata: {e}")))?;
                ...
                let content = fs::read_to_string(&abs_path)
                    .map_err(|e| ToolError::Mcp(format!("Failed to read file: {e}")))?;
            ...
            "list_directory" => {
                let dir_path = params["dir_path"]
                    .as_str()
                    .ok_or_else(|| ToolError::Mcp("Missing 'dir_path' parameter".to_string()))?;
                ...
                let read_dir = fs::read_dir(&abs_path)
                    .map_err(|e| ToolError::Mcp(format!("Failed to read directory: {e}")))?;

                for entry in read_dir.flatten() {
                    let file_type = entry
                        .file_type()
                        .map_err(|e| ToolError::Mcp(format!("Failed to get file type: {e}")))?;
            ...
            "file_info" => {
                ...
                let metadata = fs::metadata(&abs_path)
                    .map_err(|e| ToolError::Mcp(format!("Failed to get metadata: {e}")))?;
            ...
            "write_file" => {
                ...
                if let Some(parent) = abs_path.parent() {
                    fs::create_dir_all(parent).map_err(|e| {
                        ToolError::Mcp(format!("Failed to create parent directories: {e}"))
                    })?;
                }

                // Write file with content
                fs::write(&abs_path, content)
                    .map_err(|e| ToolError::Mcp(format!("Failed to write file: {e}")))?;
            ...
            "append_file" => {
                ...
                let mut file = fs::OpenOptions::new()
                    ...
                    .open(&abs_path)
                    .map_err(|e| {
                        ToolError::Mcp(format!("Failed to open file for appending: {e}"))
                    })?;

                use std::io::Write;
                writeln!(file, "{content}")
                    .map_err(|e| ToolError::Mcp(format!("Failed to append to file: {e}")))?;
            ...
            "edit_file" => {
                ...
                let line_number = params["line_number"]
                    .as_u64()
                    .ok_or_else(|| ToolError::Mcp("Missing 'line_number' parameter".to_string()))?
                    as usize;

                let new_content = params["new_content"]
                    .as_str()
                    .ok_or_else(|| ToolError::Mcp("Missing 'new_content' parameter".to_string()))?;
                ...
                fs::write(&abs_path, new_file_content)
                    .map_err(|e| ToolError::Mcp(format!("Failed to write modified file: {e}")))?;
            ...
            _ => Err(ToolError::Mcp(format!("Unknown action: {action}"))),
        }
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_filesystem_alias() {
        let kit = FileSystemKit::new();
