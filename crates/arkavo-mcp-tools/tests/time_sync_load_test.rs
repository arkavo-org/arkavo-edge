...
async fn load_test_1000_concurrent_agents() {
        ...
        move |i| {
            ...
            async move {
                ...
                if count % 100 == 0 {
                    ...
                    println!("Progress: {}/{} agents ({:.0} agents/sec)", count, agent_count, rate);
                }
    ...
    println!("Throughput: {:.1} agents/sec", agent_count as f64 / total_elapsed);
    println!("Success rate: {:.1}%", (successful as f64 / agent_count as f64) * 100.0);
...
async fn load_test_10k_requests() {
    ...
    println!("Configuration: {} rounds × {} agents = {} total requests",
    ...
    for round in 0..rounds {
        ...
        if !latencies.is_empty() {
            ...
            println!("Round {}/{}: {:.2}s, {} ok, {} fail, avg {:.2}ms",
                round + 1, rounds, round_elapsed, successful, failed, avg);
        }
    }
    ...
    metrics.print_report();
    println!("Total time: {:.2}s", total_elapsed);
    println!("Throughput: {:.1} req/sec", total_requests as f64 / total_elapsed);
...
async fn load_test_rapid_fire() {
    ...
    println!("Launching {} agents with {} max concurrency", agent_count, max_concurrency);
    ...
    println!("\n=== Peak Performance ===");
    println!("Total time: {:.2}s", total_elapsed);
    println!("Peak throughput: {:.1} agents/sec", agent_count as f64 / total_elapsed);
    println!("Success rate: {:.1}%", (successful as f64 / agent_count as f64) * 100.0);
