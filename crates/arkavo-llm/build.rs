...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("cargo:rerun-if-changed=models/Qwen3-0.6B-Q4_K_M.gguf");
    ...
    println!("cargo:rustc-cfg=feature=\"use_generated_data\"");
    
    // Model is now always embedded via include_bytes! in the source code
    // so we don't need to generate it at build time
    println!("cargo:info=Using statically embedded model via include_bytes!");
...
// We no longer need to generate model data since we use include_bytes! directly
