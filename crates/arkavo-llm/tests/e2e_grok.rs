//! End-to-end test against the **live Grok 4.7** xAI Responses API.
//!
//! Exercises the **router path** for `ModelChoice::Grok47`:
...
#[tokio::test]
#[ignore = "Requires XAI_API_KEY — makes a live Grok 4.7 Responses call"]
async fn grok47_round_trips_a_prompt() {
    let Some(client) = grok_client() else {
        eprintln!("XAI_API_KEY not set — skipping live Grok 4.7 e2e");
        return;
    };
    ...
    match result {
        ...
        Err(err) => panic!("Grok 4.7 round-trip failed: {err}"),
    }
}

#[tokio::test]
#[ignore = "Requires XAI_API_KEY — live Grok provider construction check"]
async fn grok47_provider_advertises_tools() {
