...
pub(crate) async fn generate_tokens_pooled(
    ...
    if spec_safe {
        generate_tokens_pooled_with_spec(pooled_ctx, model, prompt_bytes, config, tx).await;
        return;
    }
...
/// Spec-decoding variant of `generate_tokens_pooled_baseline`.
///
/// Runs the normal sample/decode loop against a pooled context, but at each
/// step drafts up to `N_DRAFT_MAX` tokens via NGRAM_SIMPLE, decodes a
...
#[cfg(all(feature = "llama-cpp", not(target_env = "musl")))]
async fn generate_tokens_pooled_with_spec(
    pooled_ctx: std::sync::Arc<std::sync::Mutex<LlamaContext>>,
    model: Arc<LlamaModel>,
    prompt_bytes: Vec<u8>,
    config: StreamingConfig,
...
#[cfg(all(feature = "llama-cpp", not(target_env = "musl")))]
async fn generate_tokens_with_spec(
    model: Arc<LlamaModel>,
    prompt_bytes: Vec<u8>,
    config: StreamingConfig,
    tx: UnboundedSender<Result<StreamResponse>>,
