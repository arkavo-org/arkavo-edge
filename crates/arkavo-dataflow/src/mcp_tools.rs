...
impl Tool for DiscoverLlmProvidersTool {
    async fn execute(&self, _params: Value) -> Result<Value, Box<dyn std::error::Error + Send + Sync>> {
...
impl Tool for GenerateLlmBlueprintTool {
    async fn execute(&self, params: Value) -> Result<Value, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let pipeline_type = params.get("pipeline_type")
            .and_then(|v| v.as_str())
            .unwrap_or("simple");
    ...
    fn schema(&self) -> &ToolSchema {
        static SCHEMA: once_cell::sync::Lazy<ToolSchema> = once_cell::sync::Lazy::new(|| {
            ToolSchema {
                ...
                description: "Generate a dataflow blueprint with LLM nodes for a specific task".to_string(),
                parameters: json!({
                    "type": "object",
                    "properties": {
                        "task": {
