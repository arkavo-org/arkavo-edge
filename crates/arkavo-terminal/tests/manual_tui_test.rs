...
mod manual_tests {
    ...
    fn test_tui_tools_available() {
        ...
        assert!(tools.contains(&"tui_keyboard".to_string()), "tui_keyboard tool not found");
        assert!(tools.contains(&"tui_screenshot".to_string()), "tui_screenshot tool not found");
        assert!(tools.contains(&"tui_interaction".to_string()), "tui_interaction tool not found");
        assert!(tools.contains(&"tui_harness".to_string()), "tui_harness tool not found");
    }
    ...
    fn test_tui_keyboard_schema() {
        ...
        let schema = mcp.get_tool_schema("tui_keyboard")
            .expect("Failed to get tui_keyboard schema");
        
        println!("tui_keyboard schema: {}", serde_json::to_string_pretty(&schema).unwrap());
    ...
    fn test_simple_keyboard_action() {
        ...
        match result {
            Ok(res) => {
                println!("Keyboard action result: {}", serde_json::to_string_pretty(&res).unwrap());
