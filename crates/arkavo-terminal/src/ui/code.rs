...
impl Renderable for CodeView {
    fn render(&mut self, frame: &mut ratatui::Frame, area: Rect) {
        ...
        let rendered_lines = self.render_lines();

        // Calculate visible range
        let visible_height = inner_area.height as usize;
        let start = self.scroll_offset as usize;
        let end = (start + visible_height).min(rendered_lines.len());

        if start < rendered_lines.len() {
            let visible_lines: Vec<Line> = rendered_lines[start..end].to_vec();
        ...
        if rendered_lines.len() > visible_height {
            let scroll_percentage = if rendered_lines.len() > 1 {
                (f32::from(self.scroll_offset) / (rendered_lines.len() - 1) as f32 * 100.0) as u16
            } else {
                0
            };
