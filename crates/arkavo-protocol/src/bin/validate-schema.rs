...
fn main() {
    ...
    for method in &schema.methods {
        ...
        if !method.name.chars().all(|c| c.is_lowercase() || c == '_' || c == '.') {
            errors.push(format!("Method '{}' doesn't follow naming convention (lowercase with underscores)", method.name));
        }
    ...
    let required_methods = ["promise_request", "promise_declare", "agent_discover", "rpc.discover"];
