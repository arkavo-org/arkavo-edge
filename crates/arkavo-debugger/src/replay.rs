...
impl SessionReplay {
    ...
    pub fn from_events(mut events: Vec<Event>) -> Result<Self> {
        ...
        if !events.iter().all(|e| e.session_id == session_id) {
            return Err(DebuggerError::Replay("Events from multiple sessions".to_string()));
        }
    ...
    pub fn previous(&mut self) -> Option<&Event> {
        if self.current_position > 0 {
            self.current_position -= 1;
            Some(&self.events[self.current_position])
        } else {
            None
        }
    }
    ...
    pub fn get_summary(&self) -> SessionSummary {
        ...
        for event in &self.events {
            ...
            *summary.event_types.entry(event_type.to_string()).or_insert(0) += 1;
            *summary.agents.entry(event.metadata.agent_id.clone()).or_insert(0) += 1;
...
mod tests {
    ...
    fn test_replay_navigation() {
        ...
        // Test next
        let event1 = replay.next().unwrap();
        assert_eq!(event1.sequence, 1);
        assert_eq!(replay.position(), 1);
        
        let event2 = replay.next().unwrap();
