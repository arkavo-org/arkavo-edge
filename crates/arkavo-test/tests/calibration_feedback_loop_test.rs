...
async fn test_calibration_feedback_loop() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let stream_id = log_stream_result["stream_id"]
        .as_str()
    ...
    println!(
        "   Export triggered: {}",
        export_result["message"].as_str().unwrap_or("")
    );
    ...
    if screenshot_result["success"].as_bool() == Some(true) {
        println!(
            ...
            screenshot_result["path"]
                .as_str()
                .unwrap_or("/tmp/calibration_test.png")
        );
    }
