...
mod test_idb_connection {
    ...
    async fn test_idb_connection_to_simulator() {
        ...
        eprintln!("✓ Found booted simulator: {} ({})", device.name, device.udid);
        ...
        match IdbWrapper::list_targets().await {
            Ok(targets) => {
                ...
                eprintln!("Targets: {}", serde_json::to_string_pretty(&targets).unwrap_or_default());
                
                // Check if our device is in the list
                if let Some(arr) = targets.as_array() {
                    let device_found = arr.iter().any(|t| {
                        t.get("udid").and_then(|u| u.as_str()) == Some(&device.udid)
                    });
        ...
        match IdbWrapper::tap(&device.udid, test_x, test_y).await {
            Ok(result) => {
                ...
                eprintln!("Result: {}", serde_json::to_string_pretty(&result).unwrap_or_default());
            }
            Err(e) => {
