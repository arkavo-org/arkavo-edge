...
async fn test_calibration_debug_flow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let devices = device_result.result["devices"]
        .as_array()
        .ok_or("No devices found")?;
    let device = devices.first().ok_or("No booted device found")?;
    ...
    if !app_check_output.status.success() {
        ...
        eprintln!(
            "   Install result: {}",
            serde_json::to_string_pretty(&install_result.result)?
        );
    } else {
    ...
    eprintln!(
        "   Start result: {}",
        serde_json::to_string_pretty(&start_result.result)?
    );

    let session_id = start_result.result["session_id"]
        .as_str()
    ...
    for i in 0..30 {
        ...
        let elapsed = status_result.result["elapsed_seconds"]
            .as_u64()
            .unwrap_or(0);
        let app_running = status_result.result["app_running"]
            .as_bool()
            .unwrap_or(false);

        eprintln!(
            "   [{}s] Status: {} (elapsed: {}s, app_running: {})",
            i + 1,
            status,
            elapsed,
            app_running
        ...
        if status == last_status && status == "validating" {
            ...
            if stuck_counter >= max_stuck_checks {
                eprintln!(
                    "\n   WARNING: Calibration stuck in 'validating' for {} seconds",
                    stuck_counter
                );
        ...
        .await
    {
        Ok(cal_data) => {
            if cal_data.result["success"].as_bool() == Some(true) {
                ...
                eprintln!(
                    "   Config: {}",
                    serde_json::to_string_pretty(&cal_data.result["config"])?
                );
            } else {
