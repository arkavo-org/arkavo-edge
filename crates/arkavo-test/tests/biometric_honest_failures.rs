...
async fn test_biometric_tools_return_failures_when_automation_fails() {
    ...
    eprintln!("biometric_auth result: {}", serde_json::to_string_pretty(&result).unwrap());
...
async fn test_biometric_enrollment_returns_proper_error() {
    ...
    let _ = server.call_tool(arkavo_test::mcp::server::ToolRequest {
        tool_name: "device_management".to_string(),
        params: json!({
            "action": "set_active",
    ...
    eprintln!("enrollment result: {}", serde_json::to_string_pretty(&result).unwrap());
