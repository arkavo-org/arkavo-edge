...
async fn test_calibration_stuck_debug() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let devices = device_result["devices"]
        .as_array()
    ...
    if dimensions_output.status.success() {
        if let Ok(types_json) =
            serde_json::from_slice::<serde_json::Value>(&dimensions_output.stdout)
        {
    ...
    eprintln!(
        "   Testing tap at ({}, {}) - 20%, 20% of screen",
        test_x, test_y
    );
    ...
    if simctl_tap.status.success() {
        ...
        eprintln!(
            "      ✗ simctl tap failed: {}",
            String::from_utf8_lossy(&simctl_tap.stderr)
        );
    }
    ...
    if idb_check.status.success() {
        let idb_path = String::from_utf8_lossy(&idb_check.stdout)
            .trim()
            .to_string();
        ...
        if idb_tap.status.success() {
            ...
            eprintln!(
                ...
                String::from_utf8_lossy(&idb_tap.stderr)
            );
        }
    } else {
