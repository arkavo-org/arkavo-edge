...
async fn test_xctest_quick_verify() {
    ...
    match result {
        ...
        Err(e) => {
            println!("XCTest verification error: {} - this is expected in CI environments", e);
        }
    }
}
...
async fn test_xctest_device_verification() {
            ...
            if let Some(devices_array) = device_list.as_array() {
                for device in devices_array {
                    if let Some(state) = device.get("state").and_then(|s| s.as_str()) {
                        if state == "Booted" {
                            device_id = device.get("udid").and_then(|u| u.as_str()).map(|s| s.to_string());
                            break;
                        }
                    }
                }
    ...
    let status = XCTestVerifier::verify_device(&device_id).await.expect("Verification failed");
...
async fn test_xctest_bridge_lifecycle() {
    ...
    let socket_path = std::env::temp_dir()
        .join(format!("test-xctest-bridge-{}.sock", std::process::id()));
