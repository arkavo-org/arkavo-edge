...
async fn test_mcp_server_readiness() -> Result<()> {
    ...
    let categories = [
        ...
        ("UI Interaction", vec!["ui_interaction", "ui_query", "ui_element_handler"]),
        ("Screen Capture", vec!["screen_capture", "analyze_screenshot"]),
        ("Biometric/Dialog", vec!["biometric_auth", "system_dialog", "face_id_control", "biometric_dialog_handler"]),
        ("Testing", vec!["run_test", "list_tests", "intelligent_bug_finder"]),
        ("Simulator Control", vec!["simulator_control", "simulator_advanced"]),
        ("App Management", vec!["app_management", "deep_link", "app_launcher"]),
    ];
    
    println!("\n3. Available tool categories:");
    for (category, tool_names) in &categories {
        let available: Vec<_> = tool_names.iter()
    ...
    assert!(tools.len() > 20, "Expected at least 20 tools, found {}", tools.len());
    assert!(tools.iter().any(|t| t.name == "device_management"), "device_management tool missing");
    assert!(tools.iter().any(|t| t.name == "ui_interaction"), "ui_interaction tool missing");
    assert!(tools.iter().any(|t| t.name == "screen_capture"), "screen_capture tool missing");
