...
async fn test_calibration_tap_sequence() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Extract device ID
    let device_id = if let Some(line) = device_str.lines()
        .find(|line| line.contains("(Booted)")) {
    ...
    if !launch_output.status.success() {
        eprintln!("   Failed to launch app: {}", String::from_utf8_lossy(&launch_output.stderr));
        return Ok(());
    }
