...
use arkavo_test::{TestError, Result};
use std::sync::Arc;
...
async fn test_xctest_compiler_creates_bundle() {
    ...
    // Compile bundle
    let bundle_path = compiler.get_xctest_bundle().expect("Failed to compile XCTest bundle");
    
    // Verify bundle exists
    assert!(bundle_path.exists(), "XCTest bundle should exist at {:?}", bundle_path);
...
async fn test_unix_socket_bridge_lifecycle() {
    ...
    // Start bridge
    bridge.start().await.expect("Failed to start Unix socket bridge");
...
async fn test_device_manager_finds_simulators() {
    ...
    // Refresh devices
    device_manager.refresh_devices().expect("Failed to refresh devices");
    ...
    assert!(!iphone_devices.is_empty(), "Should find at least one iPhone simulator");
    
    // Print found devices for debugging
    for device in &iphone_devices {
        eprintln!("Found device: {} ({}) - {}", device.name, device.id, device.state);
    }
}
...
async fn test_simulator_boot_and_xctest_integration() {
    ...
    device_manager.set_active_device(&iphone.id).expect("Failed to set active device");
    
    // Create XCTest compiler and bridge
    let compiler = XCTestCompiler::new().expect("Failed to create compiler");
    let bundle_path = compiler.get_xctest_bundle().expect("Failed to compile bundle");
