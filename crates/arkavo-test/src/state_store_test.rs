...
mod tests {
    ...
    fn test_update_operations() {
        ...
        let updated = store
            .update(
                "user",
                "update",
                Some(json!({"age": 31, "city": "NYC"})),
                |current, _, update_data| match (current, update_data) {
                    (Some(curr), Some(upd)) => {
    ...
    fn test_query_with_filter() {
        ...
        // Create entities
        store
            .set("user1", json!({"type": "user", "active": true}))
            .unwrap();
        store
            .set("user2", json!({"type": "user", "active": false}))
            .unwrap();
        store
            .set("admin1", json!({"type": "admin", "active": true}))
            .unwrap();
