...
impl CalibrationResults {
    pub fn average_offset(&self) -> Option<Coordinate> {
        let valid_taps: Vec<_> = self.tap_events.iter().filter(|t| t.target_hit).collect();

        if valid_taps.is_empty() {
            return None;
        }

        let sum_x: f64 = valid_taps.iter().map(|t| t.actual.x - t.expected.x).sum();

        let sum_y: f64 = valid_taps.iter().map(|t| t.actual.y - t.expected.y).sum();
...
impl VerificationReader {
    ...
    pub fn read_calibration_results(&self) -> Result<CalibrationResults, CalibrationError> {
        ...
        for path in &paths {
            ...
            if path.exists() {
                ...
                eprintln!(
                    "[VerificationReader] Parsed results: {} tap events",
                    results.tap_events.len()
                );
        ...
        Err(CalibrationError::ValidationError(
            "No calibration results found. Ensure reference app is running in calibration mode."
                .to_string(),
        ))
    }
    ...
    pub fn wait_for_results(
        &self,
        timeout_secs: u64,
    ) -> Result<CalibrationResults, CalibrationError> {
    ...
    fn get_app_documents_path(&self) -> PathBuf {
        ...
        eprintln!(
            "[VerificationReader] Getting app container path for device: {}",
            self.device_id
        );
...
pub fn apply_calibration_offset(original: &Coordinate, offset: &Coordinate) -> Coordinate {
    Coordinate {
