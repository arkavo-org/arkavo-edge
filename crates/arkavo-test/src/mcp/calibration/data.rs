...
impl CalibrationDataStore {
    ...
    pub fn store_calibration(
        ...
        eprintln!(
            "[CalibrationDataStore] Wrote calibration result to: {}",
            result_path.display()
        );
    ...
    pub fn get_latest_result(
        ...
        let result_path = self.get_result_path(device_id);
        eprintln!(
            "[CalibrationDataStore] Reading calibration result from: {}",
            result_path.display()
        );
        let data = fs::read_to_string(&result_path)?;
        let result: CalibrationResult = serde_json::from_str(&data)?;
        eprintln!(
            "[CalibrationDataStore] Retrieved result - success: {}, successful_interactions: {}/{}",
            result.success,
            result.validation_report.successful_interactions,
            result.validation_report.total_interactions
        );
    ...
    pub fn list_calibrated_devices(&self) -> Vec<DeviceSummary> {
        ...
        device_data
            .into_iter()
            .map(|(id, device_type, last_calibrated)| {
                DeviceSummary {
                    device_id: id.clone(),
                    device_type,
                    last_calibrated,
