...
impl ReferenceAppInterface {
    ...
    pub fn launch(&self) -> Result<(), CalibrationError> {
        ...
        let check_output = Command::new("xcrun")
            .args(["simctl", "get_app_container", &self.device_id, &self.bundle_id])
            .output()?;
            
        if !check_output.status.success() {
            // Reference app not installed - this is a critical error
            return Err(CalibrationError::ConfigurationError(
                format!("ArkavoReference app '{}' is not installed. Cannot proceed with calibration.", self.bundle_id)
            ));
        }
        ...
        if !output.status.success() {
            return Err(CalibrationError::ConfigurationError(
                format!("Failed to launch app: {}", String::from_utf8_lossy(&output.stderr))
            ));
        }
    ...
    pub fn navigate_to_test_screen(&self, screen_name: &str) -> Result<(), CalibrationError> {
        ...
        if !output.status.success() {
            return Err(CalibrationError::InteractionFailed(
                format!("Failed to navigate via deep link: {}", String::from_utf8_lossy(&output.stderr))
            ));
        }
        ...
        let _ = Command::new("xcrun")
            .args(["simctl", "io", &self.device_id, "tap", &x.to_string(), &y.to_string()])
            .output();
    ...
    pub fn get_calibration_script(&self) -> CalibrationScript {
        ...
        let has_reference_app = check_output
            .map(|o| o.status.success())
            .unwrap_or(false);
