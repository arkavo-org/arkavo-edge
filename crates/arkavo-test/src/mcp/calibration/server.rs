...
impl CalibrationServer {
    ...
    async fn run_calibration(&self, session_id: &str) -> Result<(), CalibrationError> {
        ...
        let (device_id, agent, reference_app) = {
            ...
            let session = sessions.get(session_id).ok_or_else(|| {
                CalibrationError::ConfigurationError("Session not found".to_string())
            })?;

            eprintln!(
                "Calibration: Found session for device {}",
                session.device_id
            );
        ...
        eprintln!(
            "Calibration: Device parameters retrieved: {} ({}x{})",
        ...
        self.update_session_status(session_id, CalibrationStatus::Validating)
            .await;
        ...
        loop {
            ...
            if start_time.elapsed() > CALIBRATION_TIMEOUT {
                eprintln!(
                    "Calibration: Timeout after {} seconds",
                    CALIBRATION_TIMEOUT.as_secs()
                );
                return Err(CalibrationError::InteractionFailed(format!(
                    "Calibration timed out after {} seconds",
                    CALIBRATION_TIMEOUT.as_secs()
                )));
            }
            ...
            eprintln!("Calibration: Attempt {calibration_attempts} of {MAX_ATTEMPTS}");
            ...
            match verification_reader.wait_for_results(10) {
                Ok(results) => {
                    ...
                    if let Some(avg_offset) = results.average_offset() {
                        ...
                        eprintln!(
                            "Calibration: No valid taps recorded, continuing with current offset"
                        );
                        break;
                    }
                }
        ...
        if calibration_result.success {
            ...
            self.update_session_status(session_id, CalibrationStatus::Complete)
                .await;
            Ok(())
        } else {
    ...
    fn generate_calibration_result_with_offset(
        ...
        interaction_adjustments.insert(
            "global".to_string(),
    ...
    pub async fn get_calibration_status(
        &self,
        session_id: &str,
    ) -> Option<CalibrationStatusReport> {
        let sessions = self.active_calibrations.read().await;
        sessions
            .get(session_id)
            .map(|session| CalibrationStatusReport {
        ...
        loop {
            {
                ...
                tokio::time::sleep(tokio::time::Duration::from_secs(
                    monitor.check_interval_hours * 3600,
                ))
                .await;
            }
        }
    ...
    async fn mark_session_failed(&self, session_id: &str, error: String) {
        self.update_session_status(session_id, CalibrationStatus::Failed(error))
            .await;
    }
...
impl CalibrationAPI {
    ...
    pub async fn handle_request(&self, request: CalibrationRequest) -> CalibrationResponse {
        match request {
            CalibrationRequest::StartCalibration {
                device_id,
                reference_bundle_id,
            } => {
                match self
                    .server
                    .start_calibration(device_id, reference_bundle_id)
                    .await
                {
                    Ok(session_id) => CalibrationResponse::SessionStarted { session_id },
                    Err(e) => CalibrationResponse::Error {
                        message: e.to_string(),
                    },
                }
            }
