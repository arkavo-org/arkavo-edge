...
impl CalibrationAgentImpl {
    ...
    fn parse_ui_element(element: &serde_json::Value) -> Option<UIElement> {
        ...
        Some(UIElement {
            ...
            accessibility_id: element["accessibilityIdentifier"]
                .as_str()
                .map(String::from),
    ...
    pub fn execute_tap(&self, x: f64, y: f64) -> Result<InteractionResult, CalibrationError> {
        ...
        eprintln!(
            "[CalibrationAgentImpl::execute_tap] Starting tap at ({}, {}) for device {}",
            x, y, self.device_id
        );
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            if device_status.as_deref() != Some("Booted") {
                eprintln!(
                    "[CalibrationAgentImpl::execute_tap] Warning: Device {} is not in 'Booted' state: {:?}",
...
impl CalibrationAgent for CalibrationAgentImpl {
    ...
    fn get_device_parameters(&self) -> Result<DeviceProfile, CalibrationError> {
        ...
        let (width, height, scale) = if let Some((w, h)) = self.query_device_dimensions() {
            ...
            let scale =
                if sim_info.device_type.contains("iPad") || sim_info.device_type.contains("SE") {
                    2.0
                } else {
    ...
    fn execute_interaction(
        ...
        match &action.action_type {
            ...
            _ => Err(CalibrationError::InteractionFailed(format!(
                "Action type {:?} not implemented yet",
                action.action_type
            ))),
        }
    }
