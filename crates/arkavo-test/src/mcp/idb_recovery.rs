...
impl IdbRecovery {
    ...
    pub async fn attempt_recovery(&self) -> Result<()> {
        ...
        eprintln!("[IdbRecovery] Current state - Companion running: {}, Port accessible: {}", 
        ...
        if companion_running && !port_accessible {
            eprintln!("[IdbRecovery] Detected companion running but port not accessible - likely stuck");
    ...
    pub async fn recover_stuck_companion(&self) -> Result<()> {
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            // Step 1: Clear all device connections from tracking
            {
                ...
                devices.clear();
                eprintln!("[IdbRecovery] Cleared {} devices from connection tracking", device_count);
            }
            ...
            if let Some(output) = lsof_output {
                if output.status.success() {
                    ...
                    for pid in pids.lines() {
                        if let Ok(pid_num) = pid.trim().parse::<i32>() {
                            eprintln!("[IdbRecovery] Killing process {} holding port 10882", pid_num);
                            let _ = Command::new("kill")
                                .arg("-9")
                                .arg(pid_num.to_string())
                                .output();
            ...
            if let Err(e) = IdbWrapper::initialize_with_preference(false) {
                ...
                if let Err(e) = IdbWrapper::initialize_with_preference(true) {
                    return Err(TestError::Mcp(format!("Failed to initialize IDB after recovery: {}", e)));
                }
            }
