...
pub fn setup_framework_links(target_dir: &PathBuf) -> Result<()> {
    ...
    if let Some(system_path) = found_path {
        ...
        let frameworks = ["FBControlCore.framework", "FBDeviceControl.framework", "FBSimulatorControl.framework"];
        ...
        for framework in &frameworks {
            ...
            if source.exists() && !target.exists() {
                eprintln!("[FrameworksData] Linking {} from {}", framework, source.display());
                std::os::unix::fs::symlink(&source, &target)
                    .map_err(|e| TestError::Mcp(format!("Failed to create symlink for {}: {}", framework, e)))?;
            }
        }
