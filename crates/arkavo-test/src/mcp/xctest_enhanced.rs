...
use super::xctest_unix_bridge::{CommandResponse, TapCommand, XCTestUnixBridge, TargetType};
...
impl XCTestEnhanced {
    ...
    fn install_test_bundle(&self, device_id: &str, bundle_path: &std::path::Path) -> Result<()> {
        let output = Command::new("xcrun")
            .args(["simctl", "install", device_id, bundle_path.to_str().unwrap()])
            .output()
    ...
    pub async fn tap_by_accessibility_id(&self, id: &str, timeout: Option<f64>) -> Result<CommandResponse> {
...
impl SimctlInteraction {
    ...
    pub fn send_swipe(device_id: &str, x1: f64, y1: f64, x2: f64, y2: f64, duration: f64) -> Result<()> {
