...
impl Tool for XCTestStatusKit {
    async fn execute(&self, params: Value) -> Result<Value> {
        ...
        let find_best = params.get("find_best").and_then(|v| v.as_bool()).unwrap_or(false);
        
        if find_best {
            // Find the best device for XCTest
            match DeviceXCTestStatusManager::find_best_xctest_device(self.device_manager.clone()).await {
                Ok(Some(best_device)) => {
                    Ok(serde_json::json!({
                        "success": true,
            ...
            // Check all devices
            match DeviceXCTestStatusManager::get_all_devices_with_status(self.device_manager.clone()).await {
                Ok(devices) => {
                    let functional_count = devices.iter()
                        .filter(|d| d.xctest_status.as_ref().map(|s| s.is_functional).unwrap_or(false))
                        .count();
                    
                    let with_bundle_count = devices.iter()
                        .filter(|d| d.xctest_status.as_ref().map(|s| s.bundle_installed).unwrap_or(false))
                        .count();
                    
                    let booted_count = devices.iter()
