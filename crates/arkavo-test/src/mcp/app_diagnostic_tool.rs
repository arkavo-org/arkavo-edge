    ...
    pub fn new() -> Self {
        Self {
            schema: ToolSchema {
                ...
                description: "Check if apps are installed and get their state on the simulator".to_string(),
        ...
        if let Some(target_bundle_id) = bundle_id {
            if let Some(apps_array) = apps.as_object() {
                for (_, app) in apps_array {
                    if let Some(app_bundle_id) = app.get("CFBundleIdentifier").and_then(|v| v.as_str()) {
