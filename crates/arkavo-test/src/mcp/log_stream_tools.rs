    ...
    async fn execute(&self, params: Value) -> Result<Value> {
        ...
        match action {
            "start" => {
                ...
                self.start_log_stream(&device_id, process_name, predicate)
                    .await
            }
            ...
            "stop" => {
                let stream_id = params
                    ...
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| {
                        TestError::Mcp("Missing stream_id for stop action".to_string())
                    })?;

                self.stop_log_stream(stream_id).await
            }

            "status" => self.get_stream_status().await,
...
impl Tool for AppDiagnosticExporter {
    async fn execute(&self, params: Value) -> Result<Value> {
        ...
        let _ = Command::new("pkill").args(["-f", "log stream"]).output();
