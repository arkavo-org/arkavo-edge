...
impl IdbErrorHandler {
    ...
    pub fn analyze_and_guide(error: &str) -> ErrorGuidance {
        ...
        if error.contains("Address already in use") || error.contains("port 10882") {
            ...
            guidance
                .suggested_fixes
                .push("Run: pkill -9 idb_companion".to_string());
            guidance
                .suggested_fixes
                .push("Or wait for auto-recovery to clean up stuck processes".to_string());
        ...
        // Device not found errors
            ...
            guidance
                .suggested_fixes
                .push("Ensure the simulator is booted".to_string());
            guidance
                .suggested_fixes
                .push("Try: xcrun simctl boot <device-id>".to_string());
            guidance
                .suggested_fixes
                .push("Verify device ID is correct".to_string());
        ...
        // Connection refused errors
            ...
            guidance
                .suggested_fixes
                .push("IDB companion may not be running".to_string());
            guidance
                .suggested_fixes
                .push("Check if port 10882 is blocked by firewall".to_string());
            guidance
                .suggested_fixes
                .push("Try restarting the IDB companion".to_string());
            guidance.can_auto_recover = true;
        }
        // Framework conflicts
            ...
            guidance
                .suggested_fixes
                .push("Install system IDB: brew install facebook/fb/idb-companion".to_string());
            guidance
                .suggested_fixes
                .push("Set: export ARKAVO_USE_SYSTEM_IDB=1".to_string());
            guidance
                .suggested_fixes
                .push("The system will attempt to use simctl as fallback".to_string());
            guidance.can_auto_recover = false;
        }
        // Binary not found
            ...
            guidance
                .suggested_fixes
                .push("Rebuild the project: cargo build".to_string());
            guidance
                .suggested_fixes
                .push("Or install system IDB: brew install facebook/fb/idb-companion".to_string());
            guidance.can_auto_recover = false;
        }
        // Security/signing issues
            ...
            guidance
                .suggested_fixes
                .push("Add Terminal to Privacy & Security > Developer Tools".to_string());
            guidance.suggested_fixes.push(
                "Or install signed system IDB: brew install facebook/fb/idb-companion".to_string(),
            );
            guidance
                .suggested_fixes
                .push("Temporary fix: sudo spctl --master-disable (not recommended)".to_string());
            guidance.can_auto_recover = false;
        }
    ...
    pub fn format_guidance(guidance: &ErrorGuidance) -> String {
        ...
        if guidance.can_auto_recover {
            output.push(
                "\n✅ Auto-recovery available - the system will attempt to fix this automatically"
                    .to_string(),
            );
        } else {
