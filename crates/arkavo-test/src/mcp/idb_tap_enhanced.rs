...
impl IdbTapEnhanced {
    ...
    pub async fn tap_with_verification(
        ...
        let companion_healthy = IdbCompanionHealth::check_health(device_id)
            .await
            .unwrap_or(false);
        ...
        for attempt in 0..max_retries {
            if attempt > 0 {
                eprintln!(
                    "[IdbTapEnhanced] Retry attempt {} of {}",
                    attempt,
                    max_retries - 1
                );
            ...
            match Self::try_idb_tap_with_timing(device_id, x, y).await {
                Ok(mut result) => {
                    ...
                    eprintln!(
                        "[IdbTapEnhanced] IDB tap succeeded on attempt {} ({}ms)",
                        attempt + 1,
                        latency
                    );
                        ...
                        if let Ok(mut result) = Self::try_simctl_tap(device_id, x, y).await {
                            ...
                            if let Some(obj) = result.as_object_mut() {
                                obj.insert(
                                    ...
                                    serde_json::json!(tap_start.elapsed().as_millis()),
                                );
                                obj.insert("attempt".to_string(), serde_json::json!(attempt + 1));
                                obj.insert(
                                    "total_time_ms".to_string(),
        ...
        eprintln!(
            "{}",
            serde_json::to_string_pretty(&IdbCompanionHealth::get_health_report())
                .unwrap_or_default()
        );
    ...
    pub async fn tap_sequence_with_verification(
        ...
        for (idx, (x, y)) in taps.iter().enumerate() {
            eprintln!(
                "[IdbTapEnhanced] Executing tap {} of {} at ({}, {})",
                idx + 1,
                taps.len(),
                x,
                y
...
mod tests {
    ...
    fn test_screen_size_inference() {
        let (w, h) = IdbTapEnhanced::infer_screen_size(
