...
impl IdbPortManager {
        ...
        if lsof_output.status.success() {
            ...
            for line in output.lines().skip(1) {
                ...
                if parts.len() > 1 && parts[0].contains("idb_compan") {
                    let pid = parts[1];
                    eprintln!("[IdbPortManager] Found idb_companion process {} on port {}, killing...", pid, port);
    ...
    pub fn get_next_idb_port() -> Result<u16> {
        ...
        if !Self::is_port_available(default_port) {
            eprintln!("[IdbPortManager] Default port {} is in use, attempting cleanup...", default_port);
        ...
        if let Some(port) = Self::find_available_port(default_port + 1, max_port) {
            ...
            Err(TestError::Mcp("No available ports for IDB companion".to_string()))
        }
    }
}