...
impl XCTestCompiler {
    ...
    fn compile_xctest_bundle(&self) -> Result<PathBuf> {
        ...
        eprintln!("XCTest bundle compiled successfully at: {}", xctest_bundle.display());
    ...
    fn process_templates(&self, source_dir: &Path) -> Result<()> {
        ...
        let swift_source = swift_template
            .replace("{{SOCKET_PATH}}", &self.socket_path.to_string_lossy());
...
"#;
    ...
    fn compile_swift_package(&self, build_dir: &Path) -> Result<()> {
        ...
        let output = Command::new("xcodebuild")
            .args(&[
                ...
                "-derivedDataPath", build_dir.join("DerivedData").to_str().unwrap(),
                "-configuration", "Debug",
        ...
        if !output.status.success() {
            ...
            let swift_output = Command::new("xcrun")
                .args(&[
                    ...
                    "-Xlinker", "-bundle",
                    "-o", build_dir.join("ArkavoTestRunner").to_str().unwrap(),
                    build_dir.join("Sources/ArkavoTestRunner.swift").to_str().unwrap(),
                ])
                .output()
    ...
    pub fn install_to_simulator(&self, device_id: &str, bundle_path: &Path) -> Result<()> {
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(TestError::Mcp(format!("Failed to install test bundle: {}", stderr)));
        }
    ...
    pub fn run_tests(&self, device_id: &str, bundle_id: &str) -> Result<()> {
        ...
        let output = Command::new("xcodebuild")
            .args(&[
                ...
                "-only-testing", &format!("{}/ArkavoTestRunner/testRunCommands", bundle_id),
            ])
            .output()
...
fn find_compiled_binary(derived_data: &Path, name: &str) -> Result<PathBuf> {
    ...
    Err(TestError::Mcp(format!("Binary {} not found in DerivedData", name)))
}