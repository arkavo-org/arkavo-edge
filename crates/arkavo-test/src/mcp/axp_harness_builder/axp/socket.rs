            ...
            for entry in entries.flatten() {
                if let Some(name) = entry.file_name().to_str() {
                    ...
                    if name.starts_with(&format!("arkavo-axp-{}-", device_id)) && name.ends_with(".sock") {
                        ...
                        eprintln!("[SocketManager] Found socket for device {}: {}", device_id, socket_path.display());
                        ...
                        if let Ok(_stream) = tokio::net::UnixStream::connect(&socket_path).await {
                            ...
                            self.update_cache(device_id.to_string(), socket_path_str.clone()).await;
                    ...
                    if let Ok(age) = now.duration_since(modified) {
                        if age > max_age {
                            ...
                            match fs::remove_file(&path) {
                                ...
                                Err(e) => {
                                    eprintln!("[SocketManager] Failed to remove stale socket {}: {}", path.display(), e);
                                }
                            }
                        }
                    }
        ...
        for entry in fs::read_dir(&self.socket_dir)? {
            ...
            if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                if name.starts_with(&format!("arkavo-axp-{}-", device_id)) && name.ends_with(".sock") {
                    match fs::remove_file(&path) {
                        Ok(_) => {
                            eprintln!("[SocketManager] Removed socket for device {}: {}", device_id, path.display());
                            removed_count += 1;
                        }
                        Err(e) => {
                            eprintln!("[SocketManager] Failed to remove socket {}: {}", path.display(), e);
                        }
                    }
                }
            }
