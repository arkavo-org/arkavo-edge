use super::calibration::server::{CalibrationRequest, CalibrationResponse, CalibrationServer};
...
impl CalibrationTool {
    pub fn new() -> Result<Self> {
        ...
        let server =
            CalibrationServer::new(storage_path).map_err(|e| TestError::Mcp(e.to_string()))?;
        ...
        match action {
            "start_calibration" => {
                ...
                if !app_check {
                    ...
                    eprintln!(
                        "[CalibrationTool] ArkavoReference app not found. Installing automatically..."
                    );
                ...
                match self.server.handle_request(request).await {
                    CalibrationResponse::SessionStarted { session_id } => Ok(json!({
                        "success": true,
            ...
            "get_status" => {
                let session_id = params["session_id"].as_str().ok_or_else(|| {
                    TestError::Mcp("session_id is required for get_status".to_string())
                })?;
                    ...
                    CalibrationResponse::Status(status) => {
                            ...
                            .and_then(|output| {
                                if output.status.success() {
                                    ...
                                    let is_running = output_str.lines().any(|line| {
                                        line.contains("UIKitApplication:com.arkavo.reference")
                                    });
                                    eprintln!(
                                        "[CalibrationTool] App running check for device {}: {}",
                                        status.device_id, is_running
                                    );
                                    Some(is_running)
                                } else {
                                    eprintln!(
                                        "[CalibrationTool] Failed to check app status via launchctl"
                                    );
                                    None
                                }
                            })
                        ...
                        if status.status == "validating" && status.tap_count < 5 {
                            if let Some(last_tap) = status.last_tap_time {
                                let seconds_since_tap =
                                    (chrono::Utc::now() - last_tap).num_seconds();
                        ...
                        match status.status.as_str() {
                            "initializing" => {
                                if !app_running {
                                    ...
                                    response["next_action"] = json!(
                                        "Check if the app is installed and try launching it manually"
                                    );
                            ...
                            "validating" => {
                                ...
                                if is_recovering {
                                    response["message"] =
                                        json!("Calibration Phase 2: Auto-recovery in progress");
                                    response["phase"] = json!({
                                        "current": 2,
                                        "name": "Auto-Recovery",
                                        "status": "recovering",
                                    ...
                                    response["next_action"] = json!(
                                        "Wait for auto-recovery to complete (usually 5-10 seconds)"
                                    );
                                } else {
                                    response["message"] = json!(format!(
                                        "Calibration Phase 2-3: Tap sequence and verification ({}s elapsed, {} taps completed)",
                                        elapsed, status.tap_count
                                    ));
                                    response["phase"] = json!({
                                        "current": 2,
                                    ...
                                    response["next_action"] =
                                        json!("Continue checking status until complete");
                                }
                            }
                            "complete" => {
                                ...
                                response["next_action"] = json!(
                                    "Use 'get_calibration' action to retrieve the calibration data"
                                );
                            }
                            "failed" => {
                                response["message"] =
                                    json!("Calibration failed. Check logs for details.");
                                response["phase"] = json!({
                                    "current": 0,
                                    "name": "Failed",
                                    "status": "error",
                                ...
                                response["next_action"] =
                                    json!("Review error details and retry calibration");
                            }
                            _ => {}
                        }
            ...
            "get_calibration" => {
                let device_id = params["device_id"].as_str().ok_or_else(|| {
                    TestError::Mcp("device_id is required for get_calibration".to_string())
                })?;
            ...
            "enable_monitoring" => {
                let enabled = params["enabled"].as_bool().ok_or_else(|| {
                    TestError::Mcp("enabled is required for enable_monitoring".to_string())
                })?;
            ...
            "export_calibration" => {
                let device_id = params["device_id"].as_str().ok_or_else(|| {
                    TestError::Mcp("device_id is required for export_calibration".to_string())
                })?;

                let export_data = self
                    .server
                    .data_store
                    .export_calibration(device_id)
                    .map_err(|e| TestError::Mcp(e.to_string()))?;
            ...
            "import_calibration" => {
                let calibration_data = params["calibration_data"].as_str().ok_or_else(|| {
                    TestError::Mcp(
                        "calibration_data is required for import_calibration".to_string(),
                    )
                })?;
                ...
                let device_id = self
                    .server
                    .data_store
                    .import_calibration(calibration_data)
                    .map_err(|e| TestError::Mcp(e.to_string()))?;
            ...
            "install_reference_app" => {
                let device_id = params["device_id"].as_str().ok_or_else(|| {
                    TestError::Mcp("device_id is required for install_reference_app".to_string())
                })?;
        ...
        if let Some(device_id) = params["device_id"].as_str() {
            ...
            if let Some(config) = self.server.data_store.get_calibration(device_id) {
                let is_valid = self
                    .server
                    .data_store
                    .is_calibration_valid(device_id, 24 * 7);
