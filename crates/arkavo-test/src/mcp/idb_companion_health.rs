    ...
    pub fn record_tap_result(device_id: &str, success: bool, latency_ms: u64) {
        ...
        if success {
            ...
            let new_avg = if metrics.total_taps_succeeded == 1 {
                ...
                let prev_total =
                    metrics.average_tap_latency_ms * (metrics.total_taps_succeeded - 1) as f64;
        ...
        eprintln!(
            "[IdbCompanionHealth] Tap {} for device {} (latency: {}ms, success rate: {:.1}%)",
