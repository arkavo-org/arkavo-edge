...
use gherkin::{Feature as GherkinFeature, ParseFileError, GherkinEnv};
...
impl Parser {
    pub fn parse_feature_file(path: &Path) -> Result<Feature> {
        ...
        let gherkin_feature = GherkinFeature::parse_path(path, env)
            .map_err(|e| match e {
                ParseFileError::Reading { path: _, source } => TestError::Io(source),
                ParseFileError::Parsing { path, error: _, source: _ } => {
    ...
    fn convert_feature(gherkin_feature: &GherkinFeature) -> Result<Feature> {
        let background = gherkin_feature.background.as_ref().map(|bg| {
            Background {
                name: if bg.name.is_empty() { None } else { Some(bg.name.clone()) },
                steps: bg.steps.iter().map(|step| {
                    Step {
                        keyword: StepKeyword::parse(&step.keyword)
                            .unwrap_or(StepKeyword::Given),
                        text: step.value.clone(),
                        data_table: step.table.as_ref().map(|table| {
                            let headers = table.rows.first()
                                .map(|row| row.to_vec())
                                .unwrap_or_default();
                            let rows = table.rows[1..].iter()
                                .map(|row| row.to_vec())
                                .collect();
        ...
        let scenarios = gherkin_feature.scenarios.iter().map(|scenario| {
            Scenario {
                name: scenario.name.clone(),
                tags: scenario.tags.to_vec(),
                steps: scenario.steps.iter().map(|step| {
                    Step {
                        keyword: StepKeyword::parse(&step.keyword)
                            .unwrap_or(StepKeyword::Given),
                        text: step.value.clone(),
