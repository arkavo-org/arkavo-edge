//! Shared `key: value` line parser for AGENTS.md.
//!
//! Both the YAML frontmatter and the markdown body use the same flat key syntax, so
//! this walks one line at a time and carries the little bit of state (which nested
//! block we are inside) that the flat keys need.
