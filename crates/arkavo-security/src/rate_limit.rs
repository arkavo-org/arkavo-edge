...
impl RateLimiter {
    ...
    pub fn check_rate_limit(&self) -> Result<()> {
        ...
        match self.limiter.check() {
            Ok(_) => Ok(()),
            Err(_) => Err(SecurityError::RateLimitExceeded),
        }
    }

    /// Check rate limit with jitter (for retry scenarios)
    pub async fn check_rate_limit_with_jitter(&self) -> Result<()> {
        if !self.config.enabled {
            return Ok(());
        }
...
/// Callback for reporting rate limiter metrics
pub type MetricsCallback = Arc<dyn Fn(usize) + Send + Sync>;
