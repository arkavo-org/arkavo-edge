...
from eval import threshold_from
...
def test_threshold_from_prefers_rewrite_rows() -> None:
    results = [
    ...
    assert threshold_from(results) == 0.77


def test_threshold_from_falls_back_to_any_confidential_method() -> None:
    results = [
        _row("unseen", "confidential", 0.65),
        _row("public", "confidential", 0.40),
        _row("internal", "internal", 0.10),
    ]
    assert threshold_from(results) == 0.40


def test_threshold_from_defaults_when_no_confidential_rows() -> None:
    results = [
        _row("public", "public", 0.05),
        _row("internal", "internal", 0.15),
    ]
    assert threshold_from(results) == 0.5
